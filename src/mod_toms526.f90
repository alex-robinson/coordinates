...
module mod_toms526
    ...
    private
    public :: interp2D_akima  

contains 

    subroutine interp2D_akima_grid_grid(x,y,z,xout,yout,zout)
        ...
        real(dp), allocatable :: x1D(:), y1D(:), z1D(:) 
        real(dp), allocatable :: xout1D(:), yout1D(:), zout1D(:) 
        ...
        ndp = size(x,1)*size(x,2)
        nip = size(xout,1)*size(xout,2)  
        allocate(iwk(max(31,27+ncp)*ndp+nip))
        allocate(wk(8*ndp))

        allocate(x1D(ndp),y1D(ndp),z1D(ndp))
        allocate(xout1D(nip),yout1D(nip),zout1D(nip))
        ...
        zout = reshape(zout1D,[size(zout,1),size(zout,2)])

        return 

    end subroutine interp2D_akima_grid_grid

    subroutine interp2D_akima_points_points(x,y,z,xout,yout,zout)
...
!         integer, parameter :: md = 1
!         integer, parameter :: ncp = 4
!         integer, parameter :: ndp = 20
!         integer, parameter :: nip = 5
...
!         real(dp) :: wk(8*ndp)
        ...
        ! Call interpolation routine 
        call idbvip (md=md,ncp=ncp,ndp=ndp,xd=x,yd=y,zd=z, &
                    nip=nip,xi=xout,yi=yout,zi=zout,iwk=iwk,wk=wk)

        return 

    end subroutine interp2D_akima_points_points 
    ...
    !
    !    Input, integer ( kind = 4 ) IPT(3*NT), the indices of the triangle
    !    vertexes.
    !
    ...
    !    numbers.
    !
    !    Input, integer ( kind = 4 ) NXI, NYI, the number of grid points in the X 
    !    and Y coordinates.
    !
    ...
    !
    !    Input/output, integer ( kind = 4 ) IWK(18*NDP), 
    !    real ( kind = 8 ) WK(8*NDP), arrays that contain information about the 
...
!           if ( nit/=0) go to 80      ! AJR: COMMENTED OUT FOR NOW
    ...
    !    the data points,
    !
    !    Input, integer ( kind = 4 ) ITI, triangle number of the triangle in which
    ...
    !    coordinates of the data points.
    !
    !    Input, integer ( kind = 4 ) NXI, NYI, the number of output grid points in
