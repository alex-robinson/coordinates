...
!     REG%alpha  = 14.5d0
    ...
    ! Output grid to file
    file_gREG = "maps/grid_"//trim(REG%name)//".nc"
    ...
    call map_init(mCCSM3_REG,gCCSM3,gREG,max_neighbors=20,lat_lim=4.0d0,fldr="maps",load=.TRUE.)
    call map_init(mREG_CCSM3,gREG,gCCSM3,max_neighbors=20,lat_lim=2.0d0,fldr="maps",load=.TRUE.)

    ! Map each field to the regional domain using the quadrant method (no max_distance required here)
    call map_field(mCCSM3_REG,"Ts",CCSM3a%Ts,REG%Ts,REG%mask,"quadrant")
    call map_field(mCCSM3_REG,"MB",CCSM3a%MB,REG%MB,REG%mask,"quadrant")
    ...
    call map_field(mREG_CCSM3,"MB",REG%MB,CCSM3b%MB,CCSM3b%mask,"shepard",125.d3)
